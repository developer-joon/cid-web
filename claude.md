...
이 파일은 Claude Code가 이 프로젝트에서 작업할 때 **반드시** 참조하는 컨텍스트입니다.
새 세션이 시작되면 이 파일을 먼저 읽고, 아래 규칙에 따라 동작합니다.
...
인증은 **LDAP 기반 로그인**을 사용하며, 최초 마일스톤은 "로그인 → 보호된 라우트 접근"까지.
...
> 스택 변경이 필요하다고 판단되면 **먼저 제안하고 승인받을 것**. 임의 변경 금지.
...
├── docs/
│   ├── prototype/             # ⚠️ UI 프로토타입 HTML. UI 작업 시 반드시 먼저 읽을 것
│   ├── architecture.md        # 아키텍처 결정 기록 (ADR)
│   └── api-spec.md            # 백엔드 연동 관련 메모
├── src/
│   ├── app/
│   │   ├── (auth)/            # 비인증 라우트 그룹 (login 등)
│   │   ├── (app)/             # 인증 필요 라우트 그룹
...
**구조 변경 원칙**: 새 폴더를 만들기 전에 위 구조에서 위치를 찾을 수 있는지 먼저 확인. 못 찾으면 제안 후 승인.
...
1. `docs/prototype/` 디렉토리를 `ls`로 확인
...
프로토타입과 **다르게 구현할 경우**에는:

- 이유를 설명하고
- 사용자에게 확인 요청
- 승인 후 `docs/architecture.md`에 결정 사항 기록
...
> 📌 **세부 방식은 `docs/architecture.md`에 정의됨**. 이 섹션은 불변 원칙만.
...
- 외부에서 들어오는 데이터(API 응답 제외, URL params, localStorage 등)는 Zod로 파싱
...
1. 관련 파일/디렉토리를 먼저 **읽고** 파악 (Read before Write)
2. UI 작업이면 `docs/prototype/` 먼저 확인 (§4)
3. API 작업이면 `src/api/generated/` 또는 OpenAPI 스펙 먼저 확인 (§5)
4. 계획을 짧게 제시하고 실행

### 8.2 중간 보고

- **큰 변경(파일 3개 이상 생성/수정)**은 phase로 쪼개서 phase마다 중간 확인 요청
- 의존성 추가 시 이유 설명 후 설치
- 새 환경변수가 필요하면 `.env.example`에도 즉시 반영
...
- ❌ 사용자가 요청하지 않은 리팩토링
- ❌ 프로토타입 디자인과 임의로 다르게 구현
- ❌ 기존 컨벤션을 무시한 새로운 패턴 도입
...
- 불확실한 부분은 추측하지 말고 **질문**
- 결정이 필요한 지점은 옵션과 trade-off를 제시하고 선택 요청
- 작업 전후에 "뭘 할 것인지 / 뭘 했는지" 짧게 요약

---

## 11. 참고 문서

- `docs/architecture.md` — 아키텍처 결정 기록 (ADR). 이 파일과 충돌 시 `architecture.md` 우선
- `docs/prototype/` — UI 프로토타입 원본
- `docs/api-spec.md` — 백엔드 연동 메모 (엔드포인트 특이사항 등)

---

**이 파일과 다르게 작업해야 하는 합당한 이유가 있다면, 먼저 제안하고 승인받은 뒤 이 파일을 업데이트할 것.**
