...
마지막 업데이트: 2026-05-08 (버그 수정 + 종합 e2e 테스트 추가)
...
| # | 항목 | 발견 | 처리 |
|---|---|---|---|
| 5.1 | `/api/v1/ips`는 **POST만 정의** — top-level GET 부재 | 사이클 #1 (스키마 점검 시) | [backend] 백엔드 추가 후 사이드바 /ip 활성. 사이클 #5에서는 CI 탭만 부분 구현 (글로벌 /ip 페이지는 보류). |
| 5.2 | IP 회수 시 `unassignCi: true` 플래그 사용 (UpdateIpRequest) | 스키마 분석 시 | 명세 명확함 — 변경 불필요 |
| 5.3 | Subnet 트리 — `/subnets/{id}/subtree` 응답 구조 미상 | 스키마 분석 시 | [compromise] 사이클 #4에서 `/subnets?page=0&size=500`으로 전체 목록 획득 후 클라이언트 트리 구성. subtree 엔드포인트는 미사용. |
| 5.4 | `/subnets` 필터 파라미터 부재 — CIDR like / VLAN ID / VRF 등 서버측 필터 없음 | 사이클 #4 | [compromise] 클라이언트 측 검색으로 대체 (CIDR·설명 포함 검색). 대용량 환경에서는 백엔드 필터 추가 필요 |
| 5.5 | Subnet 등록/편집 폼의 ciId 입력 — CI picker 없음 | 사이클 #4 | [compromise] 단순 NumberField로 대체. CI 선택 picker는 CI 검색 API 완비 후 추가 |
...
| 6.2 | History 응답의 rev 메타 (작업자, 일시, X-Change-Reason) 형식 미상 | History 사이클 #7 | [compromise] CI 이력 표시는 활성. `rev`, `revDt`, `revType`, `username`, `changeReason` 필드를 optional로 파싱. 응답 형태 가정 검증 필요. |
| 6.3 | 시점 스냅샷 응답 — `/cis/{ciId}/history/{rev}`이 단건 CI 상세 형태인지 diff 형태인지 미상 | 동상 | [compromise] CI 이력 표시는 활성. CiDetailSchema.partial().passthrough()로 CI 상세 부분집합 가정. diff 형태인 경우 별도 파싱 어댑터 필요. |
...
| #7 이력 ✅ partial | (없음 — 신규 갭 없음) | 6.2 partial ([compromise] 응답 형태 가정), 6.3 partial ([compromise] CiDetail 부분집합 가정), 9.3 완료 (이력 탭 활성; 연결맵은 다음 사이클) |
| #8 트리 master ✅ | (없음 — 가정 맞음) | (해당 없음 — 신규 도메인) |
| 버그 수정 | (없음) | 프록시 double-prefix 404 버그 수정; 사이드바 active 하드코딩 버그 수정 |
